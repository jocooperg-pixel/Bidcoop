#!/usr/bin/env python3
"""
BidCoop — Ingestion Directa del Excel Oficial de Mercado Público (Cotizaciones.xls)
...
# Detectar automáticamente el archivo más reciente de Cotizaciones en Downloads
import glob
_candidates = sorted(
    glob.glob("/Users/jonathancooper/Downloads/Cotizaciones*.xls") +
    glob.glob("/Users/jonathancooper/Downloads/Cotizaciones*.xlsx"),
    key=os.path.getmtime,
    reverse=True
)
EXCEL_PATH = _candidates[0] if _candidates else "/Users/jonathancooper/Downloads/Cotizaciones.xls"
...
CATALOG_AMINORTE = [
    ...
    "cinta embalaje", "tijera", "regla", "guillotina", "chinche", "oficina", "utiles", "útiles",
...
CATALOG_VMOCCS = [
    ...
    "estante", "librero", "cajonera", "archivo metálico", "kardex", "locker", "panel divisorio", "mueble"
]
...
def calculate_smart_catalog_match(title, desc=""):
    ...
    if match_inder > 0 and match_inder >= match_aminorte and match_inder >= match_vmoccs:
        ...
        is_tech = any(k in full_text for k in ["tóner", "toner", "impresora", "mouse", "teclado", "usb", "hdmi", "plotter", "computacional"])
...
def main():
    print(f"[{datetime.datetime.now().isoformat()}] Leyendo archivo oficial {EXCEL_PATH}...")
    ...
    print(f"[SUCCESS] Se cargaron {len(df)} Compras Ágiles oficiales desde el Excel de Mercado Público.")
    ...
    # Inserción de procesos Ground-Truth no presentes en el Excel de compras activas
    ...
    # Escribir a mockData.ts — split into parts to avoid f-string nesting issues
...
export const mockPostulaciones: Postulacion[] = [
  ...
  {{
    ...
    oportunidadTitulo: "Adquisición de Insumos y Servicios de Impresión y Corte Laser",
    organismo: "JUNTA NACIONAL DE JARDINES INFANTILES (JUNJI)",
...
"""
    ...
    # FLETES as a plain string (no f-string) to avoid brace escaping hell
    part2 = """
...
"""
    ...
    print(f"[{datetime.datetime.now().isoformat()}] [SUCCESS] Se actualizó {OUTPUT_FILE} con {len(processed)} Compras Ágiles oficiales (Regiones y Fletes corregidos).")
